...
namespace LWP {
    ...
    HubClient::HubClient(const std::optional<std::string>& name_contains) {
    ...
    void HubClient::connect(std::optional<std::string> name_contains) {
        ...
        adapter_.scan_for(std::chrono::milliseconds(3000));
        ...
        for (auto& p : peripherals) {
            try {
                ...
                bool hasLwp = false;
                for (auto& u : uuids) {
                    if (strcasecmp(u.uuid.c_str(), LWP::kServiceUUID) == 0) {
                        hasLwp = true;
                        break;
                    }
                }
                ...
                if ((hasLwp || nameOk) && p.connectable()) {
                    target = p;
                    found  = true;
                    break;
                }
        ...
        for (auto& s : peripheral_.services()) {
            if (strcasecmp(s.uuid.c_str(), LWP::kServiceUUID) == 0) {
                for (auto& c : s.characteristics) {
                    if (strcasecmp(c.uuid.c_str(), LWP::kCharUUID) == 0) {
