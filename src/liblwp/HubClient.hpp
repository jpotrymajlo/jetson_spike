...
public:
    ...
    void subscribe_notifications() {
        //observer_ = obs;
        peripheral_.notify(service_uuid_, char_uuid_, [this](SimpleBLE::ByteArray) {
...
/*
    void enable_imu_stream() {
        auto pkt = encoder_->make_enable_port_notification(
            LWP::kHubImuPort, LWP::kImuMode, LWP::kImuIntervalMs, true
        );
        write(pkt);
    }
    ...
    void disable_imu_stream() {
        auto pkt = encoder_->make_enable_port_notification(
            LWP::kHubImuPort, LWP::kImuMode, LWP::kImuIntervalMs, false
        );
        write(pkt);
    }
